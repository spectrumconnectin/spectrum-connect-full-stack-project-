...
async def request_withdrawal(user: User, amount: float) -> Dict[str, Any]:
    """
    Creator withdraws `amount` (USD) to their saved PayPal email.

    Flow (fail-safe ordering):
      1. Validate config, saved PayPal email, amount, and available balance.
      ...
      4. Call PayPal. Mark completed on success, failed on rejection (which
         frees the reserved funds again).
    """
    ...
    txn = Transaction(
        ...
        to_user_id=None,                      # leaving the platform to PayPal
        type="withdrawal",
        amount=amount,
        net_amount=amount,
        currency="USD",
        ...
        metadata=TransactionMetadata(
            description=f"Withdrawal to PayPal ({user.paypal_payout_email})",
        ),
    )
    await txn.insert()
    ...
    try:
        ...
        logger.exception("PayPal payout transport error for txn %s", tx_id)
        txn.failure_reason = f"transport_error: {e}"[:300]
        await txn.save()
        raise HTTPException(status_code=502, detail="Could not reach PayPal. Your balance is unchanged; please retry shortly.")

    if result.get("ok"):
        txn.status = "completed"
        txn.processed_at = now
        txn.completed_at = datetime.utcnow()
        txn.external_transaction_id = result.get("batch_id")
        await txn.save()
        return {
            "success": True,
            "transaction_id": tx_id,
            "amount": amount,
            "paypal_email": user.paypal_payout_email,
            "batch_id": result.get("batch_id"),
            "status": "completed",
            "message": f"${amount:.2f} is on its way to {user.paypal_payout_email}.",
        }

    # Rejected by PayPal → mark failed (frees the reserved funds).
    txn.status = "failed"
    txn.failure_reason = str(result.get("error"))[:300]
    await txn.save()
    raise HTTPException(status_code=502, detail=f"PayPal rejected the payout: {result.get('error')}")
